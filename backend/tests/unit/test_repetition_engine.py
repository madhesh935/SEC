...
def test_semantically_similar_question_detected_as_repeated():
    ...
    result = analyze_repetition("When will my daughter come?", similar_vector, recent, threshold=0.9)
...
def test_utterances_outside_time_window_are_ignored():
    old_time = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=10)
    vector = _vec(5.0)
    recent = [RecentUtterance(text="Where is my daughter?", embedding=list(vector), created_at=old_time)]

    result = analyze_repetition("Where is my daughter?", vector, recent, time_window_minutes=60, threshold=0.5)
